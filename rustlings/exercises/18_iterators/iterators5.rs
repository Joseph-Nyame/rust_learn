...
fn count_collection_iterator(collection: &[HashMap<String, Progress>], value: Progress) -> usize {
    ...
    //               { "variables2": Complete, … }, … ]
    collection.iter() // Iterate over each HashMap
        .flat_map(|map| map.values()). // Flatten all the values into one iterator
        filter(|val| **val== value). // Keep only matching values
        count()   // Count them
}
